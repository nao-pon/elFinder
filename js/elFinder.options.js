		...
		netmount: {
			...
			googledrive: {
				...
				select: function(fm){
					var self = this;
					if (self.inputs.host.find('span').length) {
						self.inputs.path.parent().prev().html(fm.i18n('FolderID'));
						fm.request({
							data : {cmd : 'netmount', protocol: 'googledrive', host: 'google.com', user: 'init', options: {id: fm.id, offline: true}},
							preventDefault : true
						}).done(function(data){
							self.inputs.host.find('span').removeClass("elfinder-info-spinner");
							self.inputs.host.find('span').html(data.body.replace(/\{msg:([^}]+)\}/g, function(whole,s1){return fm.i18n(s1,'Google.com');}));
						}).fail(function(){});
					}					
				},
				done: function(fm, data){
					...
					if (data.mode == 'makebtn') {
						...
						self.inputs.host.find('span').removeClass("elfinder-info-spinner");
						self.inputs.host.find('span').html("Google.com");
						self.inputs.host[1].value = "googledrive";
						self.inputs.user.val("done");
						self.inputs.pass.val("done");
					}
				}
